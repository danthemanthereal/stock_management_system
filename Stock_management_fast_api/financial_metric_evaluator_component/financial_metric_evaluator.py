...
def get_satisfied_and_not_satisfied_financial_metrics(financial_metrics: dict, db):
    ...
    for financial_metric_name in financial_metrics.keys():
        result = (
            db.query(ProfileMetricConfiguration, FinancialMetric)
        ...
        profile_metric_config_object, financial_metric_object = result
        if check_satisfiability(financial_metric_object, profile_metric_config_object,values):
            satisfied_financial_metrics.append(financial_metric_name)
        else:
            unsatisfied_financial_metrics.append(financial_metric_name)

        if check_satisfiability_development(profile_metric_config_object, values):
            satisfied_development_metric.append(financial_metric_name)
        else:
            unsatisfied_development_metric.append(financial_metric_name)

        if check_satisfiability_benchmark_value(financial_metric_object,profile_metric_config_object, values):
...
def check_satisfiability(financial_metric_obj: FinancialMetric, profile_metric_config_object, values: list[int]) -> bool:
    last_value = values[-1]
...
def check_satisfiability_benchmark_value(financial_metric_obj: FinancialMetric,profile_metric_config_object, values: list[int]) -> bool:
    last_value = values[-1]
    if financial_metric_obj.unit == "%":
        last_value = int(last_value * 100)
